...
const toJwkFrom = async (input, key, options={}) => {
    // assertion
    ...
    if (input === 'der' || input === 'pem') {
      return await asn1enc_toJwk(key, input, {outputPublic: options.outputPublic, passphrase: options.passphrase});
    }
    else throw new Error('UnsupportedConversion');
};
const asn1enc_toJwk = async (key, format, {outputPublic, passphrase}) => {
    // Peel the pem strings
...
const fromJwkTo = async (output = 'pem', jwkey, options={}) => {
    // assertion
...
const asn1enc_fromJwk = async (jwkey, format, {outputPublic, compact=false, encOptions}) => {
...
const decryptEncryptedPrivateKeyInfo = async (epki, passphrase) => {
    const decoded = {};
...
const encryptPBES1 = async (binKey, passphrase, algorithm, iterationCount) => {
    ...
    const salt = await getRandomBytes(8); // defined as 8 octet
...
const encryptEncryptedPrivateKeyInfo = async (binKey, options = {passphrase:''}) => {
    // default params
...
const encryptPBES2 = async (binKey, passphrase, kdfAlgorithm, prf, iterationCount, cipher) => {
    // kdf
    const pBuffer = stringToArrayBuffer(passphrase);
    const salt = await getRandomBytes(
    ...
    if (cipher === 'des-ede3-cbc') { // TODO other encryption schemes
      iv = Buffer.from(await getRandomBytes(params.encryptionSchemes[cipher].ivLength));
      const CBC = des.CBC.instantiate(des.EDE);
      ...
      iv = await getRandomBytes(params.encryptionSchemes[cipher].ivLength);
      encryptedData = Buffer.from( await jscaes_encrypt(
...
const getRandomBytes = (len) => {
    ...
    const array =new Uint8Array(nodeCrypto.randomBytes(len)); 
    return array
};
const getNodeCrypto = () => {
  return crypto
...
const pbkdf1 = async (p, s, c, dkLen, hash) => {
    ...
    seed.set(s, p.length);
    for(let i = 0; i < c; i++){
      seed = await jschash_compute(seed, hash);
    }
    return seed.slice(0, dkLen);
};
const pbkdf2 = async (p, s, c, dkLen, hash) => {
    assertPbkdf(p, s, c, dkLen, hash);
    if(typeof p === 'string') p = stringToArrayBuffer(p);
    ...
    const funcF = async (i) => {
      ...
      let u = await jschmac_compute(p, seed, hash);
      let outputF = new Uint8Array(u);
      for(let j = 1; j < c; j++){
        u = await jschmac_compute(p, u, hash);
...
const jschmac_compute = async (key, data, hash = 'SHA-256') => {
    const nodeCrypto = getNodeCrypto(); // node crypto
    const f = nodeCrypto.createHmac(jschmac_hashes[hash].nodeName, key);
...
const jscaes_encrypt = async (msg, key, {name = 'AES-GCM', iv, additionalData=new Uint8Array([]), tagLength}) => {
...
const jschash_compute = async (msg, hash = 'SHA-256') => {
...
const getJwkThumbprint = async (jwkey, alg='SHA-256', output='binary') => {
    ...
    const thumbPrintBuf = await jschash_compute(uint8json, alg);
